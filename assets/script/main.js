...
/* ══════════════════════════════════════════
   MOBILE DETECTION
   Checks if we're in scroll mode (mobile/tablet)
   vs slide mode (desktop)
══════════════════════════════════════════ */
function isMobile() {
...
function updateUI(idx) {
  ...
  if(scCur) scCur.textContent = pad(idx + 1);
  if(slideLabel) slideLabel.textContent = labels[idx];
  document.getElementById('nav').classList.toggle('solid', idx > 0);
  const arrDown = document.getElementById('arr-down');
  if(arrDown) arrDown.style.opacity = idx === TOTAL-1 ? '0' : '1';
}

/* goTo — smart: scroll on mobile, animate on desktop */
function goTo(idx, dir) {
  ...
  // DESKTOP: fullscreen slide transition
...
window.addEventListener('touchend', e => {
  ...
  if(Math.abs(dy) > 40) {
    dy > 0 ? goTo(cur_s + 1, true) : goTo(cur_s - 1, false);
  }
}, { passive: true });
...
/* ══════════════════════════════════════════
   ...
   Handles: nav links, buttons, dots, mobile menu
══════════════════════════════════════════ */
...
document.addEventListener('click', e => {
  ...
  if(!mobMenu.contains(e.target) && !menuBtn.contains(e.target)) {
    closeMobileMenu();
  }
});

/* ══════════════════════════════════════════
   MOBILE: update active nav dot on scroll
══════════════════════════════════════════ */
window.addEventListener('scroll', () => {
  ...
  // also trigger count-up when stats section is visible on mobile
  const s5 = document.getElementById('s5');
  if(s5 && window.scrollY + window.innerHeight >= s5.offsetTop + 100) {
    triggerCountUp();
  }
}, { passive: true });
...
function triggerCountUp() {
  ...
  document.querySelectorAll('.stb-n[data-n]').forEach(el => {
    ...
    const tick = () => {
      ...
      el.textContent = Math.floor(v) + (plus ? '+' : '');
      if(v < tgt) requestAnimationFrame(tick);
    };
...
['.wcard','.skc'].forEach(sel => {
  document.querySelectorAll(sel).forEach(c => {
    c.addEventListener('mousemove', e => {
      ...
      c.style.transform  = `translateY(-8px) rotateX(${-y*9}deg) rotateY(${x*9}deg)`;
...
document.querySelectorAll('.bp,.bo').forEach(b => {
  ...
  b.addEventListener('mouseleave', () => {
    b.style.transition = 'transform .5s cubic-bezier(.16,1,.3,1),all .38s';
    b.style.transform  = '';
    setTimeout(() => b.style.transition = 'all .38s', 500);
  });
});
...
document.querySelectorAll('.mc').forEach((cv, idx) => {
  ...
  (function loop() {
    ...
    const bl = ctx.createRadialGradient(W*.5,H*.5,0,W*.5,H*.5,W*.5);
    bl.addColorStop(0, `rgba(${r0},${g0},${b0},.04)`);
    ...
    t += .007;
    pts.forEach(p => {
      p.x += p.vx + Math.sin(t+p.ph)*.0005;
      p.y += p.vy + Math.cos(t+p.ph)*.0005;
      if(p.x<0) p.x=1; if(p.x>1) p.x=0;
      if(p.y<0) p.y=1; if(p.y>1) p.y=0;

      const a  = .16 + .15*Math.sin(t*1.2+p.ph);
      ...
      ctx.fillStyle = `rgba(${r},${g},${b},${a*.75})`; ctx.fill();
    });
...
(function(){
  ...
  function paw(sx){
    ...
    quad(fl,fr,tr2,tl,PAW);quad(br,bl,btl,btr,PAW);quad(bl,fl,tl,btl,PAW);quad(fr,br,btr,tr2,PAW);quad(tl,tr2,btr,btl,BODY);quad(bl,br,fr,fl,PAW);
  }
  paw(1);paw(-1);
...
(function(){
  ...
  window.addEventListener('resize',resize);resize();
  const orbs=[
    {x:.15,y:.25,rw:300,rh:230,h:340,a:.04,dx:.0001,dy:.00008},
    {x:.85,y:.7, rw:260,rh:200,h:280,a:.032,dx:-.00009,dy:.00009},
    {x:.5, y:.5, rw:380,rh:290,h:300,a:.022,dx:.00007,dy:-.00008}
  ];
  const stars=Array.from({length:120},()=>({
    x:Math.random(),y:Math.random(),
    r:Math.random()*.9+.12,
    a:Math.random()*.3+.04,
    da:(Math.random()-.5)*.0012
  }));
  let t=0;
  (function draw(){
    ctx.clearRect(0,0,W,H);
    ...
    stars.forEach((s,i)=>{
      s.a+=s.da;
      if(s.a>.38)s.da*=-1;if(s.a<.02)s.da*=-1;
      const tw=.5+.5*Math.sin(t+i*.55);
      ctx.beginPath();ctx.arc(s.x*W,s.y*H,s.r,0,Math.PI*2);
      ctx.fillStyle=`rgba(210,200,228,${s.a*tw*.5})`;ctx.fill();
    });
...
/* ══════════════════════════════════════════
   GMAIL COMPOSE — Email Me Directly
══════════════════════════════════════════ */
